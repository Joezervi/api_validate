...
def _diagnose_mismatch(po_sku: str, zervi_skus: list[str]) -> str:
    ...
    for z in zervi_skus:
        if z.lower() == po_lower and z != po_sku:
            reasons.append(
                f"Case mismatch: PO has '{po_sku}', ZERVI has '{z}'"
            )
            break
    ...
    for z in zervi_skus:
        if len(z) == len(po_sku) and z != po_sku:
            ...
            if diffs:
                reasons.append(
                    f"Character difference(s) in near‑match '{z}': "
                    + "; ".join(diffs)
                )
                break
    ...
    if clean != po_sku:
        reasons.append(
            f"Hidden/non‑ASCII characters in SKU: {hidden!r}"
        )
        ...
        for z in zervi_skus:
            if po_sku in z or z in po_sku:
                reasons.append(
                    ...
                    f"ZERVI '{z}'" if po_sku in z else
                    f"Partial match found: ZERVI '{z}' is substring of "
                    f"PO '{po_sku}'"
                )
                break
...
async def verify_products(
    ...
    async with pool.acquire() as conn:
        ...
        print(f"[Verification] Loaded {len(zervi_sku_list)} SKUs from ZERVI (product_info)")
        ...
        for item in products:
            ...
            print(f"[Verification] SKU '{po_sku}' → {'EXISTING' if matched else 'MISSING'}")
            if matched:
    ...
    print(f"[Verification] Summary: {len(existing_products)} existing, {len(missing_enriched)} missing")
    return {
